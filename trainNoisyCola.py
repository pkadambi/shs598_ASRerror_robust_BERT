...
ASR_WER_LEVELS = [0.05, 0.1, 0.15, 0.2, 0.25, 0.3] # Must be either
import argparse
...
_COLA_DIR = './data/CoLACorrupted%.2f/'
_TEACHER_LOGITS_PATH = './data/CoLA/teacherLogits.pkl'
_LEARNING_RATE = 5e-5
...
if _DISTILLATION_TEMPERATURE is None:
    DF_NAME = './results/cola_baseline_wer_sweep.csv'
    _N_EPOCHS = 3
else:
    DF_NAME = './results/cola_distillation_temp%.1f.csv' % (_DISTILLATION_TEMPERATURE)
...
for WER_LEVEL in ASR_WER_LEVELS:
    ...
    _COLA_DIR_WER = _COLA_DIR % WER_LEVEL
    ...
    train_data_path = os.path.join(_COLA_DIR_WER, 'train.tsv')
    dev_data_path = os.path.join(_COLA_DIR_WER, 'dev.tsv')
